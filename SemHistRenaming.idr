  ...
  eval h = case h of
    HI t w b c :: h' => case t of
      ...
      (Lam n t0) =>
        ...
        in
        case w of
          True => apkLam n1 t1 c h1
          False => eval (HI t1 False h1 c :: h1)
      (App t1 t2) =>
        eval (HI t1 True b h :: h)

  partial
  apkVar : (n : Name) ->  (c, h : Hist) -> Hist
  apkVar n [] h = h
  apkVar n (HI (App t1 t2) w b c :: _) h =
    eval (HI t2 False b c :: h)

  partial
  apkLam : (n1 : Name) ->  (t1 : Tm) -> (c, h : Hist) -> Hist
  apkLam n t0 [] h = h
  apkLam n t0 (HI (App t1 t2) w b c :: _) h =
    eval (HI t0 w h c :: h)
